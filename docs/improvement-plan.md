...
- **The transaction engine is the strongest asset.** Preconditions (snapshot / content hash / provenance handle), overlap preflight, tree-sitter syntax preflight, atomic temp-file+rename writes with in-process reverse recovery for internal commit failures, mutation-journal rotation and crash recovery, and non-destructive validation reporting. All edit tools funnel into one `changes[]` pipeline compiled to `PlannedFile { path, before, after }`, a clean compile target for future operations.
...
| D4 | Historical validation-rollback inconsistency. **Superseded on 2026-07-13:** validation is now deliberately non-destructive in both foreground and background paths; failed validation preserves the edit. | `workspace/edit.rs`, `workspace/validation.rs` |
...
4. **D4** ✅, then superseded on 2026-07-13: validation no longer triggers rollback or validator cancellation. Failed validation is reported with the edit preserved, long validation may continue in the background, and `rollback_on_failure` remains only as a deprecated ignored compatibility field. Internal reverse recovery still protects partial commits and journal/index failures.
